    ...
    public class confirmController {
        ...
        private void showForgotPasswordDialog() {
            ...
            if (isValidEmail(mail.getText())) {
                ...
                Utilisateur user = su.getUserByEmail(mail.getText());

                if (user != null) {
                    // Generate a unique token or code and store it in the database with the user's ID and a timestamp
                    String token = generateUniqueToken();
                    long timestamp = System.currentTimeMillis();
                    su.insertPasswordResetToken(user.getIdUser(), token, timestamp, mail.getText());
                    ...
                    sendPasswordResetEmail(user.getEmailUser(), token);
        ...
        private void sendPasswordResetEmail(String email, String code) {
            ...
            Session session = Session.getInstance(properties, new Authenticator() {
                ...
                protected PasswordAuthentication getPasswordAuthentication() {
                    return new PasswordAuthentication("fbenrabii1@gmail.com", "aufqfyumqefgkpel");
                }
            });
            try {
                Message message = new MimeMessage(session);
                message.setFrom(new InternetAddress("fbenrabii1@gmail.com"));
